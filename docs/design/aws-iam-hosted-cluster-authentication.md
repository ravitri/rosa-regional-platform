...
| `rosa-regional-platform-cli` | `internal/commands/cluster/kubeconfig.go` | `rosactl cluster kubeconfig` command (includes `--region` in get-token args) |
  ...
  - name: my-cluster-iam
    user:
      exec:
        ...
        args:
          [cluster, get-token, --cluster-id, <cluster-id>, --region, <region>]
```
